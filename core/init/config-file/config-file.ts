...
import type { DenoHerderConfig } from './models.ts';

export class ConfigFile implements IAsyncConfig {
  #file: IFile;
  #logger: Logger;
  #name: string;
  #lasValue: DenoHerderConfig;
  #init = false;
  #at?: number;
  ...
  updateValue(
    value:
      | DenoHerderConfig
      | ((e: DenoHerderConfig) => DenoHerderConfig),
  ): this {
  ...
  async readFileConfig() {
    ...
    const config = JSON.parse(content) as DenoHerderConfig;
  ...
  async writeFileConfig(config?: DenoHerderConfig) {
  ...
  static async create(
    ...
    name: string,
    path: string | URL,
    initFile?: Partial<DenoHerderConfig>,
  ) {
